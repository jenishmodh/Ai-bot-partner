    ...
    if request.POST:
        ...
        try:
            ...
            if int(valid.phone) == int(number1):
                print(email1)
                request.session['user_name'] = valid.id
                ...
                msg['Subject'] = "Your OTP From AiBot Partner"
...
def password(request):
    ...
    if 'user_name' in request.session.keys():
        if request.POST:
            no1 = request.POST['pass1']
            no2 = request.POST['pass2']
            if str(no1 == str(no2)):
                data = Home_page.objects.get(id = int(request.session['user_name']))
                data.password = no2
                data.cpassword = no1
                data.save()
                return redirect('log')
